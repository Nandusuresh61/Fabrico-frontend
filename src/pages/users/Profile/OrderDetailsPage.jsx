...
import { ShoppingBag, Eye, XCircle, AlertTriangle, Package, Truck, CheckCircle, CreditCard, Receipt, Download, ArrowLeftRight, Home, ShoppingCart } from 'lucide-react';
...
const OrderDetailsPage = () => {
  ...
  return (
    <div className="mb-6 space-y-4 rounded-lg p-4 bg-white shadow-lg">
      ...
      {orders.length > 0 ? (
        <div className="overflow-hidden rounded-lg border border-gray-200 max-w-full">
            ...
            <tbody className="divide-y divide-gray-200 bg-white">
              {orders.map((order) => (
                <tr key={order.orderId} className="transition-colors hover:bg-gray-50">
                  ...
                  <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-500">{formatDate(order.createdAt)}</td>
                  ...
                  <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-500">{formatCurrency(order.totalAmount)}</td>
                  <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-500">{order.items.length}</td>
        ...
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-gray-300 py-12">
          ...
          <div className="mt-6 flex gap-4">
            <Link to="/" className="inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-medium text-white hover:bg-primary/90">
              <Home className="h-4 w-4" /> Return to Home
            </Link>
            <Link to="/products" className="inline-flex items-center gap-2 rounded-md bg-white px-4 py-2 text-sm font-medium text-gray-700 border border-gray-300 hover:bg-gray-50">
      ...
      <div className="flex justify-center gap-4 mt-6">
        <Link to="/" className="inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-medium text-white hover:bg-primary/90 transition-colors">
          <Home className="h-4 w-4" /> Return to Home
        </Link>
        <Link to="/products" className="inline-flex items-center gap-2 rounded-md bg-white px-4 py-2 text-sm font-medium text-gray-700 border border-gray-300 hover:bg-gray-50 transition-colors">
          <ShoppingCart className="h-4 w-4" /> Continue Shopping
        </Link>
      </div>
